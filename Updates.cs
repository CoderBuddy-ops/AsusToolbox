    ...
    public partial class Updates : RForm
    {
        ...
        private void RenderTable(List<UpdatesController.DriverUpdate> updates, TableLayoutPanel table)
        {
            foreach (var update in updates)
                AddDriverRow(update, table, isBiosRow: table == tableBios);
        ...
        private void AddDriverRow(UpdatesController.DriverUpdate driver, TableLayoutPanel table, bool isBiosRow = false)
        {
            ...
            {
                // BIOS updates are never treated like ordinary drivers: verify
                // model + power, then require explicit confirmation before sending
                // the user to the official ASUS installer page.
                if (isBiosRow && !ConfirmBiosUpdate(driver))
                    return;
        ...
        /// <summary>
        /// BIOS-update safety gate. Verifies the exact model, AC power and battery
        /// level, then requires an explicit confirmation dialog before the user is
        /// sent to the official ASUS installer. Never silent, never automatic.
        /// </summary>
        private bool ConfirmBiosUpdate(UpdatesController.DriverUpdate driver)
        {
            try
            {
                string detectedModel = AppConfig.GetModel();
                var modelCheck = BiosSafety.VerifyModel(detectedModel);

                bool onAc = SystemInformation.PowerStatus.PowerLineStatus == PowerLineStatus.Online;
                float lifePercent = SystemInformation.PowerStatus.BatteryLifePercent;
                decimal battery = HardwareControl.batteryCapacity > 0
                    ? HardwareControl.batteryCapacity
                    : (decimal)lifePercent;
                var powerCheck = BiosSafety.VerifyPower(onAc, battery);

                if (!modelCheck.Safe)
                {
                    MessageBox.Show(this, modelCheck.Reason, "Asus — BIOS update blocked", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    Logger.WriteLine($"BIOS update blocked: {modelCheck.Reason}");
                    return false;
                }

                if (!powerCheck.Safe)
                {
                    MessageBox.Show(this, powerCheck.Reason, "Asus — BIOS update blocked", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    Logger.WriteLine($"BIOS update blocked: {powerCheck.Reason}");
                    return false;
                }

                string message = BiosSafety.ConfirmationMessage(modelOk: true, onAc, battery) +
                                 $"\n\nUpdate: {driver.title} {driver.version}";

                var result = MessageBox.Show(this, message, "Asus — BIOS update", MessageBoxButtons.YesNo, MessageBoxIcon.Warning);
                Logger.WriteLine(result == DialogResult.Yes
                    ? $"BIOS update confirmed by user: {driver.title} {driver.version}"
                    : "BIOS update declined by user");
                return result == DialogResult.Yes;
            }
            catch (Exception ex)
            {
                Logger.WriteLine("BIOS confirmation error: " + ex.Message);
                return false;
            }
        }
