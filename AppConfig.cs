...
public static class AppConfig
{
    ...
    static AppConfig()
    {
        ...
        string appPath = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "Asus");
        string startupConfig = Path.Combine(Application.StartupPath.Trim('\\'), configName);

        fallbackConfigFile = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.CommonApplicationData), "Asus", configName);
    ...
    // Test support: point configuration at an isolated file and reset all in-memory
    // state, so tests never read or write the real user configuration.
    internal static void UseConfigFile(string path)
    {
        lock (configLock)
        {
            timer.Stop();
            configFile = path;
            fallbackConfigFile = path; // disable fallback sync
    ...
    /// <summary>
    /// Synchronously writes the current configuration to disk. Normal sets are
    /// debounced (2s) to avoid hammering the disk, but callers that persist a
    /// value and then exit immediately (e.g. the self-updater's pending marker)
    /// must Flush so the write survives the process.
    /// </summary>
    ...
    /// <summary>
    /// The validated exact target model for this build: X1504ZA. Windows can
    /// report the same hardware as "Vivobook_ASUSLaptop X1504ZA", "ASUS X1504ZA"
    /// or plain "X1504ZA", so comparison normalises separators and casing.
    /// Returns false on any other model so unrelated Vivobooks (e.g. X1502ZA,
    /// or a bare "X1504") are not treated as validated devices.
    /// </summary>
    public static bool IsX1504ZA() => ModelMatches(GetModel(), "X1504ZA");

    /// <summary>
    /// Pure model matching: splits a model string into tokens on non-alphanumeric
    /// separators and requires an exact, case-insensitive token match. This avoids
    /// the prefix ambiguity of plain substring matching ("X1504" must not match
    /// "X1504ZA") and is fully unit-testable without WMI.
    /// </summary>
    public static bool ModelMatches(string modelString, string model)
    {
        string target = NormalizeModel(model);
        if (target.Length == 0) return false;

        var sb = new System.Text.StringBuilder();
        foreach (char c in modelString)
        {
            if (char.IsLetterOrDigit(c))
                sb.Append(char.ToUpperInvariant(c));
            else
            {
                if (sb.Length > 0 && sb.ToString() == target) return true;
                sb.Clear();
            }
        }
    ...
    static string NormalizeModel(string s)
    {
    ...
    /// <summary>
    /// EC firmware version reported by Windows. ASUS firmware reports an
    /// invalid "255.255" placeholder on models whose EC version is not exposed;
    /// that value is normalised to null so the UI never shows it as real.
    /// </summary>
    public static string? GetECVersion()
    {
        try
        {
            ...
            foreach (var obj in searcher.Get())
            {
                using (obj)
                {
                    // Win32_BIOS exposes EC version via the "Version" property on
                    // some ASUS firmware; when absent, treat as unavailable.
                    string? ec = obj["Version"]?.ToString();
                    if (string.IsNullOrWhiteSpace(ec)) return null;
                    ec = ec.Trim();
                    return IsInvalidECVersion(ec) ? null : ec;
                }
            }
        }
        catch (Exception ex)
    ...
    /// <summary>255.255 (and the 255.255.255.255 form) are invalid/unavailable placeholders.</summary>
    public static bool IsInvalidECVersion(string? version)
    {
        if (string.IsNullOrWhiteSpace(version)) return true;
        string v = version.Trim().Replace(" ", "");
        return v == "255.255" || v == "255.255.255.255" || v == "0.0" || v == "0.0.0.0";
    }
    ...
    private static void Init()
    {
        ...
        // Default to a clean, modern dark UI. Users can still opt into light or
        // "Windows" mode and a non-flat look via their config.
        config["ui_mode"] = "dark";
        config["theme"] = "flat";
    ...
    /// <summary>Snapshot of all config keys (used by profile listing and diagnostics).</summary>
    public static List<string> Keys()
    {
        lock (configLock) return config.Keys.ToList();
    }
    ...
    /// <summary>
    /// Resets all configuration to defaults (used by the Settings page "Reset
    /// configuration" action). Hardware values are re-applied on next mode set;
    /// the app continues running with safe defaults.
    /// </summary>
    ...
    {
        return ContainsModel("M140") || ContainsModel("S550") || ContainsModel("K650") || ContainsModel("P540");
    }

    public static bool IsOLED()
    {
        return ContainsModel("OLED") || ContainsModel("M7600") || ContainsModel("UX64") || ContainsModel("UX34") || ContainsModel("UX53") || ContainsModel("K360") || ContainsModel("X150") || ContainsModel("M340") || ContainsModel("M350") || ContainsModel("K650") || ContainsModel("UM53") || ContainsModel("K660") || ContainsModel("UX84") || ContainsModel("M650") || ContainsModel("M550") || ContainsModel("M540") || ContainsModel("K340") || ContainsModel("K350") || ContainsModel("M140") || ContainsModel("S540") || ContainsModel("S550") || ContainsModel("M7400") || ContainsModel("N650") || ContainsModel("HN7306") || ContainsModel("H760") || ContainsModel("UX5406") || ContainsModel("M5606") || ContainsModel("X513") || ContainsModel("N7400") || ContainsModel("UX760") || ContainsModel("Q530VJ") || ContainsModel("TP3407") || ContainsModel("HT7407") || ContainsModel("3607") || _oledFromRegistry.Value;
    }
    ...
    {
        return ContainsModel("G614") || ContainsModel("G615") || ContainsModel("G634") || ContainsModel("G635") || ContainsModel("G814") || ContainsModel("G815") || ContainsModel("G834") || ContainsModel("G835");
    }
    ...
    {
        return ContainsModel("GV301RA") || ContainsModel("GV302XA") || ContainsModel("GZ302") || IsOnlyAIMAX();
    }

    public static bool NoGpu()
    {
        // X1504ZA is an Intel-Iris-Xe-only ultrabook (no discrete GPU): skip the
        // NVIDIA/AMD probe at startup and hide GPU-mode controls.
        return Is("no_gpu") || ContainsModel("X1504Z") || ContainsModel("UX540") || ContainsModel("M560") || ContainsModel("GZ302") || IsOnlyAIMAX();
    }
    ...
    {
        return ContainsModel("GU405") || ContainsModel("GU606") || ContainsModel("H760") || ContainsModel("GA403") || ContainsModel("GU605") || ContainsModel("GA605") || ContainsModel("GA503R");
