...
namespace Asus.Battery
{
    public static class BatteryControl
    {
        ...
        public static void ToggleEnergySaver()
        {
            SetEnergySaver(!IsEnergySaver());
        }

        public static void SetEnergySaver(bool enable)
        {
            AppConfig.Set("energy_saver", enable ? 1 : 0);
            try
            {
                if (enable)
                    Mode.PowerNative.SetPowerMode(Mode.PowerNative.GetDefaultPowerMode(AsusACPI.PerformanceSilent));
                else
                    Mode.PowerNative.SetPowerMode(Mode.PowerNative.GetDefaultPowerMode(AsusACPI.PerformanceBalanced));
            }
            catch (Exception ex)
            {
                Logger.WriteLine($"SetEnergySaver failed: {ex.Message}");
            }
            Program.settingsForm?.VisualiseEnergySaver();
        }

        public static bool IsEnergySaver()
        {
            return AppConfig.Is("energy_saver") || Mode.PowerNative.GetBatterySaverStatus();
        }
